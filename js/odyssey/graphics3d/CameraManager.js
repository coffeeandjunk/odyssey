...
define(
	[
		'odyssey/NameSpace',
		'jquery',
		'odyssey/graphics3d/OrbitLinesManager',
		'odyssey/graphics3d/TracerManager',
	...
	function(ns, $, OrbitLinesManager, TracerManager, Dimensions, Gui, ExportValues) {
		...
		var toggleCamera = function() {
			...
			if(lookFromBody){
				...
				if(viewSettings.lookFrom == ORBITAL_CAMERA_TYPE){
					...
					currentCamera.odyssey && currentCamera.odyssey.controls && (currentCamera.odyssey.controls.enabled = true);
				}
			}
		...
		var updateCamera = function() {
			...
			var lookAtBody = bodies3d[viewSettings.lookAt];
			var controls = currentCamera.odyssey && currentCamera.odyssey.controls;
			if(controls) {
				controls.update();
		...
		var disableControls = function(){
			_.each(allCameras, function(cam){
				cam.odyssey && cam.odyssey.controls && (cam.odyssey.controls.enabled = false);
			});
		};
		...
		var CM = {
			...
			kill : function(){
				...
				_.each(allCameras, function(cam){
					cam.odyssey && cam.odyssey.controls && cam.odyssey.controls.removeEventListener('change', onControlsUpdate);
				});
			}
